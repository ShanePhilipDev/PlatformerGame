...
void PauseMenu::Update(float frame_time)
{
...
void PauseMenu::Render()
{
	...
	level_->Render(); // Render the level behind the menu.
...
void PauseMenu::Init(gef::SpriteRenderer* sr, gef::Font* f, gef::Platform* p, GameState* gs, gef::InputManager* im, gef::AudioManager* am, Level* l, MainMenu* mm)
{
	// Set pointers.
	sprite_renderer_ = sr;
	font_ = f;
...
void PauseMenu::ProcessTouchInput()
{
	// Get touch input.
...
void PauseMenu::ProcessKeyboardInput()
{
	// Get keyboard input.
...
void PauseMenu::ProcessControllerInput()
{
...
void PauseMenu::RenderButtons()
{
	...
	font_->RenderText(
		sprite_renderer_,
		gef::Vector4(platform_->width() * 0.1f, platform_->height() * 0.3f, 0.0f),
		1.0f,
		button_colour_[0],
		gef::TJ_LEFT,
		"RESUME");
	...
	font_->RenderText(
		sprite_renderer_,
		gef::Vector4(platform_->width() * 0.1f, platform_->height() * 0.8f, 0.0f),
		1.0f,
		button_colour_[5],
		gef::TJ_LEFT,
		"EXIT");
...
void PauseMenu::InitButtons()
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		menu_buttons_[i].set_width(250);
		menu_buttons_[i].set_position(platform_->width() * 0.225f, platform_->height() * (0.35 + (0.1 * i)), 0);
	}
	...
	for (int i = 6; i < 11; i++)
	{
		...
		else
		{
			...
			if (i % 2 == 0)
			{
				menu_buttons_[i].set_position(platform_->width() * 0.6f, platform_->height() * (0.5 + (0.05 * (i - 7))), 0);
			}
			else
			{
				menu_buttons_[i].set_position(platform_->width() * 0.8f, platform_->height() * (0.45 + (0.05 * (i - 7))), 0);
			}
		}
	}
...
bool PauseMenu::IsInside(const gef::Sprite& sprite, const gef::Vector2& point)
{
...
void PauseMenu::SelectionUp()
{
	...
	else if (settings_)
	{
		...
		if (selection_ < 6) // If they escape settings menu back into main menu, return to settings.
		{
			selection_ = 6;
		}
	}
...
void PauseMenu::SelectionDown()
{
	...
	else if (settings_)
	{
		...
		else if (selection_ < 6) // If they escape settings menu back into main menu, return to settings.
		{
			selection_ = 6;
		}
	}
...
void PauseMenu::Back()
{
	...
	else // If already at main part of the menu, resume.
	{
		game_state_->SetGameState(State::LEVEL);
	}
}
